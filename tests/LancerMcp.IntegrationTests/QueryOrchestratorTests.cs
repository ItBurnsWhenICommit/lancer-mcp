...
public class QueryOrchestratorTests : FixtureTestBase
{
    ...
    public async Task Database_ShouldContainIndexedData()
    {
        ...
        repos.Should().Contain("lancer-mcp");
    }

    [Fact]
    public async Task GetRepositoryStats_LancerMcp_ShouldHaveSymbols()
    {
        // Arrange & Act
        var stats = await GetRepositoryStatsAsync("lancer-mcp");

        // Assert
        stats.SymbolCount.Should().BeGreaterThan(0, "lancer-mcp should have indexed symbols");
        stats.FileCount.Should().BeGreaterThan(0, "lancer-mcp should have indexed files");

        Console.WriteLine($"lancer-mcp stats:");
    ...
    public async Task EdgeRepository_GetOutgoingEdges_ShouldFindRelationships()
    {
        ...
        var symbol = await QueryAsync<Symbol>(@"
            ...
            LIMIT 1",
            new { QualifiedName = "LancerMcp.Services.QueryOrchestrator.QueryAsync%" });
