...
This directory contains integration tests for the Lancer MCP server.
...
docker compose exec -T postgres pg_restore -U postgres -d $TEST_DB_NAME < ../tests/fixtures/dumps/lancer_latest.dump
...
|----------|---------|-------------|
| `TEST_DB_NAME` | `lancer_test` | Test database name |
...
1. **lancer-mcp** - This project (self-test)
   - ~50-100 C# files
...
# Or manually:
cd database
docker compose exec -T postgres psql -U postgres -c "DROP DATABASE lancer_test;"
...
4. **Verify fixtures are loaded:**
   ```csharp
   await VerifyDatabaseHasData();
   VerifyGitMirrorExists("lancer-mcp");
   ```
