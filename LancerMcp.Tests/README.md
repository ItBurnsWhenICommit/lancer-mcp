...
Unit tests for the Lancer MCP server.
...
**Note:** These tests use mock implementations (`MockRepositoryRepository`, `MockBranchRepository`) that store data in-memory dictionaries. For real PostgreSQL integration tests, see `tests/LancerMcp.IntegrationTests/GitTrackerIntegrationTests.cs`.
